...
let available_domains_str =
  match List.map get_name Domains.available_domains with
...
let get_domain s =
  if s = "" then
    ...
    try List.find (fun d -> get_name d = s) Domains.available_domains
    with Not_found ->
...
let _ =
  ...
  let speclist = Arg.align [
    ("--abstract-domain", Arg.String decl_domain,
     "<domain>  Use abstract domain <domain> " ^ available_domains_str);
  ...
  try 
	...
	in
        Analyze.analyze dom !descending f !output_file
